...
export default function ThreadDetails({ thread, userId, isInstructor, onBack }) {
    ...
    const fetchReplies = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${API_BASE}/threads/${thread.id}/posts`, {credentials: 'include'});
    ...
    const handlePostReply = async (e) => {
        ...
        try {
            const response = await fetch(`${API_BASE}/threads/${thread.id}/posts`, {
                method: 'POST',
    ...
    const handleDeleteReply = async (replyId) => {
        ...
        try {
            const response = await fetch(`${API_BASE}/posts/${replyId}`, {
                method: 'DELETE',
                credentials: 'include',
            });
                                            ...
                                            <div className="w-100">
                                                ...
                                                <div className='d-flex align-items-center gap-2 mb-2'>
                                                    ...
                                                    <span className="fw-semibold text-dark small">
                                                       {reply.profiles?.name || `User #${reply.user_id}`} 
                                                    </span>
                                                    {reply.profiles?.role && (
                                                        <span className={`badge rounded-pill text-capitalize ${
                                                            reply.profiles.role === 'instructor' ? 'bg-success' : 'bg-primary'
                                                            }`} style={{ fontSize: '0.7rem' }}> 
                                                            {reply.profiles.role}
                                                        </span>
                                                    )}
                                                </div>
                                                <p className="text-dark mb-0 fs-6" style={{ whiteSpace: 'pre-line' }}>{reply.reply_message}</p>
                                            </div>
