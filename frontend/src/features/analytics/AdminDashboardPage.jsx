...
export default function AdminDashboardPage() {
  const [activeTab, setActiveTab] = useState('instructors'); // 'instructors', 'reviews', or 'users'
  const [instructors, setInstructors] = useState([]);
                        ...
                        <tbody>
                          {studentsList.length > 0 ? (
                            studentsList.map((stud, index) => {
                              // ✅ Look up all possible user mapping properties across schemas
                        ...
                        <tbody>
                          {instructorsList.length > 0 ? (
                            instructorsList.map((inst, index) => {
                              // ✅ Look up all possible instructor mapping properties across schemas
                              const instructorId = inst.instructor_id || inst.user_id || inst.id || inst.uuid;
                              
                              return (
                                <tr key={instructorId || `inst-${index}`}>
                                  <td><span className="fw-bold text-dark">{inst.name || 'Anonymous Instructor'}</span></td>
