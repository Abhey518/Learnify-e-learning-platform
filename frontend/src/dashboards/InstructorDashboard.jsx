...
export default function InstructorDashboard() {
  ...
  // QUIZZES STORAGE ENGINE MAPPING
  ...
  // --- REVIEWS & FEEDBACK STATE ENGINE ---
  const [showReviewsModal, setShowReviewsModal] = useState(false);
  const [activeCourseReviews, setActiveCourseReviews] = useState(null);
  const [courseReviewsList, setCourseReviewsList] = useState([]);
  const [courseAverageRating, setCourseAverageRating] = useState(0);
  const [reviewsLoading, setReviewsLoading] = useState(false);
  const [courseRatingsSummaryMap, setCourseRatingsSummaryMap] = useState({}); // Stores at-a-glance card ratings
      ...
      .then(coursesData => {
        ...
        Promise.all(
          (coursesData || []).map(async (course) => {
            try {
              const res = await fetch(`${baseUrl}/courses/${course.id}/reviews`);
              if (res.ok) {
                const ratingData = await res.json();
                if (ratingData.success || ratingData.average_rating !== undefined) {
                  summaryMap[course.id] = {
                    average: ratingData.average_rating || 0,
                    count: ratingData.total_reviews || (ratingData.reviews ? ratingData.reviews.length : 0)
                  };
                }
              }
            } catch (e) {
              console.warn(`Could not cache summary review values for course ID ${course.id}:`, e);
            }
          })
        ).then(() => setCourseRatingsSummaryMap(summaryMap));
      })
  ...
  // --- CURRICULUM MANAGEMENT (MODULES, LESSONS & QUIZZES) ---
  const openCurriculum = (course) => {
    setActiveCourse(course);
    setModules([]);
    setLessonsMap({});
    setQuizzesMap({}); // Clear quiz engine cache entries
    setCurriculumError('');
    ...
    fetch(`${baseUrl}/courses/${course.id}/modules`, { credentials: 'include' })
      ...
      .then(modulesData => {
        ...
        modulesData.forEach(mod => {
          fetchLessons(mod.id);
          fetchQuizzes(mod.id); // ✅ Fetch quizzes for every active module container frame
        });
      })
  ...
  const openReviewsPanel = (course) => {
    setActiveCourseReviews(course);
    setCourseReviewsList([]);
    setCourseAverageRating(0);
    setShowReviewsModal(true);
    setReviewsLoading(true);

    fetch(`${baseUrl}/courses/${course.id}/reviews`, { credentials: 'include' })
      .then(res => res.ok ? res.json() : Promise.reject("Failed to sync review entries."))
      .then(data => {
        setCourseReviewsList(data.reviews || []);
        setCourseAverageRating(data.average_rating || 0);
      })
  ...
  // FETCH QUIZZES METHOD ENGINE
  const fetchQuizzes = (moduleId) => {
    fetch(`${baseUrl}/courses/modules/${moduleId}/quizzes`, { credentials: 'include' })
      .then(res => res.ok ? res.json() : Promise.reject("Failed to load quizzes"))
      .then(quizzesData => setQuizzesMap(prev => ({ ...prev, [moduleId]: quizzesData || [] })))
      .catch(err => setQuizzesMap(prev => ({ ...prev, [moduleId]: [] })));
  };
  ...
  return (
    <div className="bg-light min-vh-100 py-5 text-dark">
      <div className="container" style={{ marginTop: '2rem' }}>
                    ...
                    <div className="card shadow-sm border-0 rounded-3 bg-white h-100 p-3">
                      <div className="card-body d-flex flex-column p-2">
                        ...
                        <div className="d-flex justify-content-between align-items-start mb-3">
                          ...
                          <div className="d-flex align-items-center mb-2 gap-1" style={{ fontSize: '0.85rem' }}>
                            {courseRatingsSummaryMap[course.id]?.count > 0 ? (
                              <span className="text-warning fw-bold">
                                <i className="bi bi-star-fill me-1"></i>
                                {Number(courseRatingsSummaryMap[course.id].average).toFixed(1)} 
                                <span className="text-muted fw-normal"> ({courseRatingsSummaryMap[course.id].count} reviews)</span>
                              </span>
                            ) : (
                              <span className="text-muted fst-italic small"><i className="bi bi-star me-1"></i>No feedback yet</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="mt-4 d-flex flex-column gap-2 border-top pt-3">
                          ...
                          <button 
                            className="btn btn-sm btn-outline-dark rounded-pill fw-medium px-3 w-100 mt-1" 
                            onClick={() => openReviewsPanel(course)}
                          >
                            <i className="bi bi-chat-left-heart-fill me-2 text-muted"></i>View Performance Reviews
                          </button>
                              ...
                              <div id={`collapse${module.id}`} className="accordion-collapse collapse" data-bs-parent="#curriculumAccordion">
                                <div className="accordion-body bg-light border-top">
                                  ...
                                  <ul className="list-group list-group-flush mb-3 rounded-3 shadow-sm border overflow-hidden">
                                    ...
                                    {/* --- QUIZZES APPEAR UNDERNEATH LESSONS STACK --- */}
                                    {quizzesMap[module.id]?.map(quiz => (
                                      <li key={quiz.id} className="list-group-item d-flex justify-content-between align-items-center bg-white py-3 border-start border-4" style={{ borderLeftColor: '#0d6efd !important' }}>
                                        <div>
                                          <span className="badge bg-primary text-white rounded-pill me-3 px-3">
                                            <i className="bi bi-patch-question me-1"></i>Quiz {quiz.order_no || ''}
                                          </span>
                                          <span className="fw-bold text-dark">{quiz.title}</span>
                                        </div>
                                        <div className="btn-group">
                                          {/* Direct link routing pass for quiz edit environments */}

                                          <button className="btn btn-sm btn-outline-primary px-3 fw-medium" onClick={() => navigate(`/courses/${activeCourse.id}/quizzes/${quiz.id}?view=marks`)}>
                                            View Marks
                                          </button>
                                          
                                          <button className="btn btn-sm btn-outline-primary px-3 fw-medium" onClick={() => navigate(`/courses/${activeCourse.id}/quizzes-list/${module.id}`)}>
                                            Edit Quiz
                                          </button>
                                          <button className="btn btn-sm btn-outline-danger px-3 fw-medium" onClick={() => navigate(`/courses/${activeCourse.id}/quizzes-list/${module.id}`)}>
                                            Delete
                                          </button>
                                        </div>
                                      </li>
                                    ...
                                    {/* Fallback view indicator checks if both stacks are empty */}
                                    {(!lessonsMap[module.id] || lessonsMap[module.id].length === 0) && (!quizzesMap[module.id] || quizzesMap[module.id].length === 0) && (
                                      <li className="list-group-item text-muted fst-italic bg-white py-4 text-center">No units or evaluations loaded into this segment.</li>
                                    )}
                                  </ul>

                                  {/* PARALLEL ACTION BUTTONS WRAPPER DESIGN CONTAINER */}
                                  <div className="text-end mt-2 d-flex justify-content-end gap-2">
                                    <button 
                                      className="btn btn-sm rounded-pill px-4 fw-medium text-white shadow-sm" 
                                      style={{ backgroundColor: '#6f42c1' }} 
                                      onClick={() => { 
                                    ...
                                    <button 
                                      className="btn btn-sm rounded-pill px-4 fw-medium text-white shadow-sm" 
                                      style={{ backgroundColor: '#6f42c1' }} 
                                      onClick={() => navigate(`/courses/${activeCourse.id}/quizzes-list/${module.id}`)}
                                    >
                                      <i className="bi bi-patch-question-fill me-1"></i> Add Quiz
                                    </button>
                                  </div>
        ...
        {/* ANONYMOUS REVIEWS INSPECTION DRAWER PANEL OVERLAY */}
        {showReviewsModal && activeCourseReviews && (
          <div className="modal fade show d-block" tabIndex="-1" style={{ backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1050 }}>
            <div className="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
              <div className="modal-content border-0 shadow-lg rounded-4">
                <div className="modal-header bg-white border-bottom-0 pt-4 px-4 pb-0">
                  <div>
                    <h5 className="modal-title fw-bold text-dark mb-0">Student Insights Evaluation Log</h5>
                    <p className="text-muted small mb-0">Course: {activeCourseReviews.title}</p>
                  </div>
                  <button type="button" className="btn-close mb-auto" onClick={() => setShowReviewsModal(false)}></button>
                </div>
                ...
                <div className="modal-body p-4">
                  {reviewsLoading ? (
                    ...
                    <>
                      {/* Aggregate metrics header banner */}
                      <div className="p-3 border rounded-3 bg-light text-center mb-4 d-flex justify-content-around align-items-center">
                        <div>
                          <span className="text-muted small fw-bold text-uppercase d-block">Aggregate Rating</span>
                          <h2 className="fw-bold text-dark m-0"><i className="bi bi-star-fill text-warning me-1"></i>{Number(courseAverageRating).toFixed(1)} / 5.0</h2>
                        </div>
                        <div className="border-start ps-4">
                          <span className="text-muted small fw-bold text-uppercase d-block">Submissions Total</span>
                          <h2 className="fw-bold text-dark m-0">{courseReviewsList.length} reviews</h2>
                        </div>
                      </div>

                      {/* Anonymous Reviews Feed */}
                      <h6 className="fw-bold mb-3 text-dark border-bottom pb-2">Anonymous Feed Responses</h6>
                      <div className="d-flex flex-column gap-3">
                        {courseReviewsList.length === 0 ? (
                          <div className="text-center py-4 text-muted fst-italic">
                            No evaluations logged by students for this curriculum profile track yet.
                          </div>
                        ) : (
                          courseReviewsList.map((rev, idx) => (
                            <div className="p-3 border rounded-3 bg-white shadow-sm" key={rev.id || idx}>
                              <div className="d-flex justify-content-between align-items-center mb-2">
                                <span className="badge bg-secondary-subtle text-secondary border px-3 py-1.5 rounded-pill small">
                                  <i className="bi bi-incognito me-1.5"></i>Verified Anonymous Student
                                </span>
                              ...
                              <p className="mb-0 text-muted small lh-base" style={{ whiteSpace: 'pre-wrap' }}>
                                "{rev.comment || 'No text commentary provided.'}"
                              </p>
                            </div>
                          ))
                        )}
